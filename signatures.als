...
/**
 * Kafka instance represents the state of the Cluster
 * This model considers a single Kafka cluster (hence singleton)
 */
one sig Kafka {
	-- Fix the number of replicas that should be present for each partition
	replicationFactor: one Int,

	-- One zookeeper to manage cluster metadata
	zookeeper: one ZooKeeper,
...
/**
 * A Broker is a single server in a cluster of servers which store replicas of Topic partitions
 */
sig Broker {
...
/**
 * Topic is the entitity that stores streamed data points in its partitions
 */
sig Topic {
...
/**
 * Represents a partition of a topic which stores published events in a sequential manner
 */
sig TopicPartition {
	-- Leader cannot be shared by any other partition
	var leader: disj set PartitionReplica,
...
sig PartitionReplica {
	...
	-- replicas cannot be shared between two parititons
...
sig KafkaEvent {} {
	...
	all p : TopicPartition | disj[p.(leader+followers).events.elems, (TopicPartition - p).(leader+followers).events.elems]
}


/**
 * ClientApp is either a Producer or Consumer
 */
abstract sig ClientApp {
	-- A client app can be assigned to multiple partitions
	-- Multiple client apps can be assigned to a partition
 ...
 * Consumer consumers KafkaEvents
 */
sig Producer, Consumer extends ClientApp {}


/**
 * A ConsumerGroup represents a group of Consumer instances
 * It subscribes to certain topics and maintains Consumer -> TopicPartition assignment 
...
sig ConsumerGroup {
	...
	-- The group maintains the index of the TopicPartition that it last read from
	var offsets: TopicPartition -> {i : Int | i >= 0}, 

	-- Consumers are assigned to partitions
	assignments: Consumer -> TopicPartition
} {
	-- CG keeps track of last read index of all partitions of subscribed topics 
	-- The offset cannot be greater than the number of events in the parition.
	all p : subscribedTo.partitions | one offsets[p] and offsets[p] <= p.leader.events.lastIdx[]
